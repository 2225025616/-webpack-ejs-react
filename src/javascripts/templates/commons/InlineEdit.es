...
require('./jquery.auto-grow-input');
...
class InlineEdit extends React.Component {
    ...
    startEditing() {
        if(this.props.editable){
            if(this.props.onXfieldFocus) this.props.onXfieldFocus();
    ...
    finishEditing() {
        if(this.props.editable){
            if(this.props.onXfieldBlur) this.props.onXfieldBlur();
    ...
    componentWillReceiveProps( nextProps ) {
        this.setState({
            text: nextProps.text,
            minLength: nextProps.minLength || 1,
            maxLength: nextProps.maxLength || 256
        });
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.state.editing && !prevState.editing) {
            ...
            $(inputElem).autoGrowInput({ minWidth: 5, maxWidth: 200, comfortZone: 12 });
