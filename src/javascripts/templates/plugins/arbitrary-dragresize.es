...
(function() {
    ...
    if (CKEDITOR.plugins.get('arbitrary-dragresize')) {
        return;
    }
     ...
     */
    CKEDITOR.plugins.add('arbitrary-dragresize', {
        onLoad: function() {
            if (!isWebkit) {
                return;
            }
    ...
    ImgDrag.prototype = {
        ...
        selectionChangeHandle: function(startElement){
        ...
        downHandle: function(e){
            ...
            this.window.addEventListener('mouseup', this.events.upHandle, false);
            // for firefox ie9 || less than ie9
            window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty();
        },

        moveHandle: function(e){
            var e_diffX = e.clientX - this.position.clientX,
                e_diffY = e.clientY - this.position.clientY,
                x = e_diffX + this.position.offsetLeft,
                y = e_diffY + this.position.offsetTop;
